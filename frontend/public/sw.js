...
const CACHE_NAME = 'easyx-offline-v14';
const APP_VERSION = '3.5.0'; // Network-first + Concorre MicroXisto + Admin Inline Edit
...
async function handleNavigation(request) {
  console.log('🏠 SW: Handling navigation (network-first)');
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('🌐 SW: Served fresh HTML from network');
      return response;
    }
  } catch (error) {
    console.log('⚠️ SW: Network failed, trying cache');
  }
...
// OFFLINE APIs - Network-first com fallback cache
async function handleOfflineAPI(request) {
  const url = new URL(request.url);
  console.log('🔄 SW: Handling API (network-first):', url.pathname);
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('🌐 SW: API from network');
      return response;
    }
  } catch (error) {
    console.log('⚠️ SW: API network failed, trying cache');
  }
  
  // Fallback: cache
  const cached = await caches.match(request);
  if (cached) {
    console.log('📱 SW: API from cache (offline)');
    return cached;
  }
...
// STATIC ASSETS - JS, CSS, imagens (Network-first)
async function handleStaticAsset(request) {
  console.log('📁 SW: Handling static asset (network-first)');
  ...
  try {
    ...
    console.log('⚠️ SW: Static asset network failed');
  }
